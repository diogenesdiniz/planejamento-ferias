...
**Pegadinha encontrada em produção, em duas rodadas:**

1. As fotos carregavam em `localhost` mas não em `diogenesdiniz.github.io`. Causa: a chave era restrita por referer com o path completo (`.../roteiro-ferias-2026/*`), mas o navegador manda só a origem (sem path) em requisições cross-origin como `fetch`/`<img>` para `places.googleapis.com` — a Maps JavaScript API aceitou esse referer mais curto, a Places API (New) não. Corrigido *inicialmente* com `referrerPolicy: 'unsafe-url'` no `fetch` e no atributo `referrerPolicy` do `<img>`, em `FotosLocal.jsx`, forçando o navegador a mandar a URL completa da página.
2. Essa correção funcionava no Chrome (desktop e Android), mas não no Safari/iOS: o `referrerPolicy` não era respeitado de forma confiável, a chamada falhava silenciosamente (tratada igual a "sem foto") e o bloco de fotos aparecia e desaparecia. Correção definitiva: em vez de forçar via código o navegador a mandar a URL completa, ampliamos a restrição da chave no Google Cloud Console para aceitar também a **origem sem o path** (`https://diogenesdiniz.github.io/*`, além do padrão mais específico já existente) — exatamente o que todo navegador manda por padrão, Safari incluso. Com isso, o `referrerPolicy: 'unsafe-url'` deixou de ser necessário e foi removido de `FotosLocal.jsx`, tanto no `fetch` quanto no `<img>`. Testado e confirmado pelo usuário funcionando em Android e iOS reais depois da correção.
